...
// The donations need to be tallied up and reporte for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
...
// Problem 1
let companyYodo = [];
companyYodo = runners.filter(word => word.company_name === "Yodo");
console.log(companyYodo);
// Problem 2
let emails = [];
runners.forEach(function (element) {
...
// Problem 3
let skinix = [];
let totalDonationSkinix = [];
skinix = runners.filter(word => word.company_name === "Skinix");
let skinixDonations = [];
skinix.forEach(function (element) {
    skinixDonations.push(element.donation);
})
totalDonationSkinix = skinix.reduce(function (accumulator, value) {
    return accumulator + value.donation;
}, 0);

console.log(totalDonationSkinix);